...
# from odoo.tools import hfctml_escape
...
class AdmissionInfo(models.Model):
    ...
    # patient_id = fields.Many2one('patient.info', string='Patient Name',readonly=True)
    ...
    admission_diagnosis = fields.Char(string="Patient Diagnosis")
    ...
    def btn_cancel_form_show(self):  # This function start to cancell form show ------
        ...
        template_id = self.env.ref('general_hospital_v_03.view_cancel_model_form').id
    ...
    def action_user_notification(self):
        title ="Please open this admission to check validity"
        action = self.env.ref('general_hospital_v_03.activate_admission_action_window')
        return {
            ...
            'params': {
                ...
                'message': '%s',
                'links': [{
                    'label': self.admission_id,
                    'url': f'#action={action.id}&id={self.admission_id}&model= admission.info'
                }],
                'sticky': False,
            }
        }
    ...
    def add_payment_btn(self):
        ...
        # ir_model_data = self.env['ir.model.data']
        # template_id = ir_model_data.ref('general_hospital_v_03.bill_pay_form_view').id
    ...
    # def add_payment_btn(self):  # This Function is used to Release Button------
    #     if self.release_note:
    #         raise ValidationError("This Admission is Already Noted!")
    #     view_id = self.env.ref('general_hospital_v_03.release_note_popup_view_form').id
    #     return {
