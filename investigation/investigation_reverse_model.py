...
class BillRegisterWizard(models.TransientModel):
    ...
    @api.depends('bill_register_line_id.total_price', 'bill_register_line_id.sub_total_amount',
                 'other_discount', 'paid', 'due_amount')
    ...
    def default_get(self, fields_list):
        ...
        if 'referred_by' in fields_list:
            defaults['referred_by'] = bill_data.referred_by.id if bill_data.referred_by else False

        if 'referral' in fields_list:
            defaults['referral'] = bill_data.referral.id if bill_data.referral else False
        if 'patient_name' in fields_list:
            defaults['patient_name'] = bill_data.patient_name.id if bill_data.patient_name else False

        for field in field_mappings.keys():
            if field in fields_list:
                defaults[field] = getattr(bill_data, field_mappings[field])
