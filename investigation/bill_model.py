...
class BillRegister(models.Model):
    ...
    _rec_name = 'bill_id'
    _order = 'bill_id desc'
    _description = 'Hospital Investigation'
    ...
    admission_id = fields.Many2one(comodel_name='admission.info', string='Admission ID')
    patient_name = fields.Many2one(comodel_name='patient.info', tracking=True)
    ...
    referred_by = fields.Many2one(comodel_name='doctors.profile', string="Referred By", domain=[('state', '=', 'active')])
    referral = fields.Many2one(comodel_name='broker.profile', string="Referral", domain=[('state', '=', 'active')])
    delivery_date = fields.Date("Delivery Date")
    # Cancel fields added here ------
    cancel_ids = fields.One2many(comodel_name='cancel.appointment', inverse_name='cancel_bill_id', string='Cancellation Records')
    ...
    card_amount = fields.Integer(string="Card Amount")
    mfs_amount = fields.Integer(string="MFS Amount")
    ...
    total_without_discount = fields.Float(string="Total Without Discount", digits='Discount',compute='_onchange_total_amount')
    discount_percent = fields.Float(string="Discount Percent(%)", digits='Discount', default=0.0)
    service_charge = fields.Float(string="Service Charge", default=0.0)
    ...
    due_amount = fields.Float(string="Due Amount", digits='Discount', default=0.0)
    bill_payments = fields.One2many(string="Payments", comodel_name='bill.bill_pay', inverse_name='bill_id')
    currency_id = fields.Many2one(string="Currency", comodel_name='res.currency', default=lambda self: self.env['res.currency'].search([('name', '=', 'BDT')]))
    ...
    def add_discount(self):#normal bttuon action
        pass
...
class BillPaymentLines(models.Model):
    ...
    billpayment_item_id = fields.Many2one('bill.register', ondelete='cascade')
